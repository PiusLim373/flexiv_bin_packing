...
BoxFinder::BoxFinder() : Node("box_finder_node")
{
  ...
  this->declare_parameter<int>("averaging_tries", 10);
  ...
  RCLCPP_INFO(this->get_logger(), "plane_distance_threshold_global: %f", plane_distance_threshold_global);
  RCLCPP_INFO(this->get_logger(), "min_cluster_size_global: %d", min_cluster_size_global);
  ...
  RCLCPP_INFO(this->get_logger(), "averaging_tries: %d", averaging_tries);
  RCLCPP_INFO(this->get_logger(), "min_averaging_tries: %d", min_averaging_tries);
                                        ...
                                        const std::shared_ptr<bin_packing_msgs::srv::GetFMBoxDetail::Response> response)
{
  ...
  for (int i = 0; i < averaging_tries; i++)
  {
    ...
    {
      RCLCPP_INFO(this->get_logger(), "No boxes detection, continuing");
      continue;
    }
  ...
  if (successful_detection < min_averaging_tries)
  {
    RCLCPP_INFO(this->get_logger(), "Less then %d successful detection, returning false", min_averaging_tries);
...
std::vector<pcl::PointIndices> BoxFinder::get_clusters(double max_pass_filter, double voxel_grid_size, double plane_distance_threshold, int min_cluster_size, bool enable_plane_segmentation, pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud_without_plane)
{
  ...
  if (enable_plane_segmentation)
  {
    RCLCPP_INFO(this->get_logger(), "Plane segmentation enabled");
  ...
  {
    RCLCPP_INFO(this->get_logger(), "Plane segmentation disabled");
    cloud_without_plane = cloud_filtered;
  }
                                         ...
                                         const std::shared_ptr<bin_packing_msgs::srv::GetAllBoxDetail::Response> response)
{
  ...
  for (int i = 0; i < boxes_detected; i++)
  {
    ...
    RCLCPP_INFO(this->get_logger(), "item%d_%f_%f", i, temp.pose.position.x, temp.pose.position.y);
    temp.shape.name = "item" + std::to_string(i) + "@_" + std::to_string(center_x) + "_" + std::to_string(center_y);
